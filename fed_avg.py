...
def select_models_on_divergence(local_models, glob_model, model_select_num, max_divergence):
    ...
    sorted_keys = sorted(divergence_dict.keys(), key=divergence_dict.get, reverse=max_divergence)  # maximum divergence
    logger.debug("sorted local uuid by local models' divergence: {}".format(sorted_keys))
    return sorted_keys[:model_select_num]


def select_models_on_entropy(local_models, glob_model, model_select_num):
    entropy_dict = {}
    for local_uuid in range(1, env_store.args.num_users + 1):
    ...
    sorted_keys = sorted(entropy_dict.keys(), key=entropy_dict.get, reverse=True)  # maximum entropy
    logger.debug("sorted local uuid by local models' entropy: {}".format(sorted_keys))
    return sorted_keys[:model_select_num]


def select_models_on_acc(local_models_acc, model_select_num):
    logger.debug("local models' accuracies: {}".format(local_models_acc))
    sorted_keys = sorted(local_models_acc.keys(), key=local_models_acc.get, reverse=True)  # maximum accuracy
    logger.debug("sorted local uuid by local models' accuracies: {}".format(sorted_keys))
    return sorted_keys[:model_select_num]


def select_models_on_loss(local_models_loss, model_select_num):
    logger.debug("local models' losses: {}".format(local_models_loss))
    sorted_keys = sorted(local_models_loss.keys(), key=local_models_loss.get)  # minimum loss
...
def receive_global_w(trainer_uuid, w_glob_compressed):
    ...
    # load hash of nb15 global model, which is downloaded from the leader
