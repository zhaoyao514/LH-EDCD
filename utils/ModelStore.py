...
class CentralModelStore:
    ...
    def local_models_add_count(self, local_uuid, w_local, acc_local, loss_local, count_target):
        reach_target = False
        lock.acquire()
        self.local_models[local_uuid] = w_local
        self.local_models_acc[local_uuid] = acc_local
        self.local_models_loss[local_uuid] = loss_local
...
class AsyncCentralModelStore(CentralModelStore):
    ...
    def async_acc_alpha_add(self, local_uuid, acc_alpha_map):
        self.acc_alpha_maps[local_uuid] = acc_alpha_map
        logger.debug("Received acc_alpha_map from user {}.".format(local_uuid))
